...
The approval API is a **host-side library boundary**, not an MCP tool. The host must authenticate the operator, protect SQLite write access, derive the authority independently of the proposal, and present the patch and staged manifest for review before passing its reviewed digest. The library cannot prove those inputs came from a human or trusted policy source. The ledger is not signed portable evidence, and a caller with write access to its database can forge or alter it. Place the database outside the watched Git worktree.

## Trust and limits

- The workspace fields are observed through local Git commands. In the proposal, `repositoryId`, `actionId`, `policyDigest`, path scope, and patch bytes remain caller inputs. The approval ledger compares them with host-supplied authority but cannot authenticate that authority's provenance; an integration with the trusted catalog, persisted plan, policy, and operator identity remains necessary.
...
The next slice must integrate trusted plan/operator provenance and design a safe compare-and-swap promotion without writing a checked-out branch behind its worktree. See the [roadmap](ROADMAP.md).
