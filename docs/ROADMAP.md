...
Current slice: a plan-only Git change proposal API and structural schema capture a clean-worktree witness, exact patch digest, and exact path list; drift and malformed-path tests exist. It does not authenticate the caller-supplied action/policy references, parse patch effects, approve, or promote a change. See [Git change proposal v1](GIT_CHANGE_PROPOSAL.md).
...
2. **Staged execution and promotion:** create the candidate change in an isolated worktree or equivalent constrained staging environment, not directly in the protected target. Compute the changed-path manifest and patch digest from trusted observation. Recheck the approval, policy, base/ref witness, paths, and patch at promotion; serialize revocation with this check and use a compare-and-swap Git ref update or fail closed. State any residual cross-system atomicity limit. A direct-write backend may produce an observation receipt but cannot receive the stronger pre-commit claim.
